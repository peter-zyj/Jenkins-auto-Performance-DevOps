...
async def verSevCheck(condition, ip, ver, build):
    ...
    with await condition:
        ...
        if okTag[ip]:
            print ("verSevCheck::Start:{}".format(ip))
        else:
            print ("verSevCheck::Ping failed {}".format(ip))
            return 1
...
def serviceCHK(condition,ip,ver):
    print ("serviceCHK::Start:{}".format(ip))
    serviceDict = {}
    serviceDict[ip] = {}
    servicepath = dir+"/"+ver+"/"+"services"
    contServ = ""
